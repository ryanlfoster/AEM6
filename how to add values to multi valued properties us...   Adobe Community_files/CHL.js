var s_tc_CHL=new TagContainer('CHL');
...
var s_chl_account="mxchlprod"
var s_chl=s_gi(s_chl_account)
s_chl.debugTracking=false
s_chl.charSet = "UTF-8"
var lThanks;
var radio_event;

/* Link and ClickMap tracking */
s_chl.trackDownloadLinks=true
s_chl.trackExternalLinks=true
s_chl.trackInlineStats=true
s_chl.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx"
s_chl.linkInternalFilters="javascript:,adobe.com"
s_chl.linkLeaveQueryString=false
s_chl.linkTrackVars='campaign,channel,events,eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar9,list1,list2';


s_chl.usePlugins=true
function s_chl_doPlugins(s) {
	s_chl.eVar4=s_chl.getQueryParam('q');
	s_chl.campaign=s_chl.pageName;
	s_chl.plugins='';
	s_chl.eVar55=s_chl.eVar56=s_chl.eVar57=s_chl.eVar58='';
	if (s_chl.eVar2 == "" && (s_chl.events.indexOf('event1') >= 0 || s_chl.events.indexOf('event2') >= 0)) {
		s_chl.abort = true;
	}

	if(s_chl.prop6){
		s_chl.eVar13 = s_chl.prop6;
	}
}
s_chl.doPlugins=s_chl_doPlugins
s_chl.visitorNamespace = "adobe"
s_chl.trackingServer = "stats.adobe.com"
s_chl.trackingServerSecure = "sstats.adobe.com"

/* Plugin: getQueryParam 2.3 */
s_chl.getQueryParam=new Function("p","d","u",""
+"var s_chl=this,v='',i,t;d=d?d:'';u=u?u:(s_chl.pageURL?s_chl.pageURL:s_chl.wd.locati"
+"on);if(u=='f')u=s_chl.gtfs().location;while(p){i=p.indexOf(',');i=i<0?p"
+".length:i;t=s_chl.p_gpv(p.substring(0,i),u+'');if(t){t=t.indexOf('#')>-"
+"1?t.substring(0,t.indexOf('#')):t;}if(t)v+=v?d+t:t;p=p.substring(i="
+"=p.length?i:i+1)}return v");
s_chl.p_gpv=new Function("k","u",""
+"var s_chl=this,v='',i=u.indexOf('?'),q;if(k&&i>-1){q=u.substring(i+1);v"
+"=s_chl.pt(q,'&','p_gvf',k)}return v");
s_chl.p_gvf=new Function("t","k",""
+"if(t){var s_chl=this,i=t.indexOf('='),p=i<0?t:t.substring(0,i),v=i<0?'T"
+"rue':t.substring(i+1);if(p.toLowerCase()==k.toLowerCase())return s_chl."
+"epa(v)}return ''");
...
function hideForm(){
	$('#helpful-pod').fadeOut(1000, function(){ 
	$('.thankyou').toggle().html( lThanks );
	$('.thankyou').delay(3000).fadeOut('slow');
	});                                    
}

s_chl.setTagContainer("CHL")
